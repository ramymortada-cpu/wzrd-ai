...
export const feedbackRouter = router({
  ...
  createFeedback: protectedProcedure.input(z.object({ deliverableId: z.number(), comment: z.string().min(1).max(5000), rating: z.number().min(1).max(5).optional() })).mutation(async ({ input, ctx: _ctx }) => createDeliverableFeedback(input)),
  ...
  publicCreate: publicProcedure.input(z.object({ deliverableId: z.number(), authorName: z.string().min(1).max(255), comment: z.string().min(1).max(5000), parentId: z.number().optional(), version: z.number().optional() })).mutation(async ({ input, ctx: _ctx }) => createDeliverableComment({ ...input, authorType: 'client' })),
  publicSubmit: publicProcedure.input(z.object({ deliverableId: z.number(), decision: z.enum(["approved", "changes_requested"]), reason: z.string().max(5000).optional(), clientName: z.string().min(1).max(255), version: z.number().optional() })).mutation(async ({ input, ctx: _ctx }) => createDeliverableApproval(input)),
});
