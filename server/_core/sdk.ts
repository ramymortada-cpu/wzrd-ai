...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      logger.warn({}, '[OAuth] OAUTH_SERVER_URL is not configured');
    }
  }
...
class SDKServer {
  ...
  async getUserInfo(accessToken: string): Promise<GetUserInfoResponse> {
    ...
    const platformVal = (data as { platform?: string })?.platform ?? null;
    const loginMethod = this.deriveLoginMethod(platforms, platformVal);
    return {
      ...data,
